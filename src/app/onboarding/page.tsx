...
export default function OnboardingPage() {
    ...
    const handleComplete = async () => {
        ...
        try {
            ...
            // Get profile to find current tenant
            ...
            if (!formData.skipItems && formData.firstItemName && formData.firstItemPrice && location) {
                const { data: item, error: itemError } = await supabase
                    .from('items')
                    .insert({
                        tenant_id: tenantId,
                        name: formData.firstItemName,
                        unit_price: parseFloat(formData.firstItemPrice),
                        cost_price: parseFloat(formData.firstItemPrice) * 0.6, // 40% margin
                        category: 'General',
                ...
                if (item && location) {
                    ...
                    if (quantityError) {
                        console.error('Quantity error:', quantityError)
                    }
                }
            }
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 py-12">
